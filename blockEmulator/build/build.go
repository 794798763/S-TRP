...
func initConfig(nid, nnm, sid, snm uint64) *params.ChainConfig {
	...
	for i := uint64(0); i < snm; i++ {
		...
		for j := uint64(0); j < nnm; j++ {
			params.IPmap_nodeTable[i][j] = "127.0.0.1:" + strconv.Itoa(8800+int(i)*200+int(j))
		}
	}
...
func BuildSupervisor(nnm, snm, mod uint64) {
	...
	lsn.NewSupervisor(params.SupervisorAddr, initConfig(255, nnm, 255, snm), params.CommitteeMethod[mod], measureMod...)
